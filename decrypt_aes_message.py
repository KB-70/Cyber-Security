...
def dec_decrypt_byte_as_nibbles( nibble_quadruple, key_quadruple ):
    ...
    the_nibbles = dec_xor(nibble_quadruple, key_quadruple)
    the_nibbles = dec_swap_last_two(the_nibbles)
    the_nibbles = dec_s_box(the_nibbles)
    the_nibbles = dec_xor(the_nibbles, key_quadruple)
    the_nibbles = dec_subtract_last_two_first_two(the_nibbles)
    the_nibbles = dec_swap_last_two(the_nibbles)
    the_nibbles = dec_s_box(the_nibbles)
    the_nibbles = dec_xor(the_nibbles, key_quadruple)
   
    return( bytearray( the_nibbles ) )
...
def dec_xor( nibbles, key ):
    ...
    # STUDENTS YOU MUST WRITE THIS FUNCTION
    xor_nibble1 = nibbles[0] ^ key[0]
    xor_nibble2 = nibbles[1] ^ key[1]
    xor_nibble3 = nibbles[2] ^ key[2]
    xor_nibble4 = nibbles[3] ^ key[3]

    ans = bytearray( [ xor_nibble1, xor_nibble2, xor_nibble3, xor_nibble4 ] )
...
def dec_swap_last_two( nibbles ):
   ...
   # STUDENTS YOU MUST WRITE THIS FUNCTION    
    ...
    ans = bytearray( [ nibs1, nibs2, nibs4, nibs3 ] )
   ...
   # STUDENTS YOU MUST WRITE THIS FUNCTION
    ...
    for nib in nibbles:
        if nibbles[index] == 0:
            nibbles[index] = 2
            index+=1
        elif nibbles[index] == 1:
            nibbles[index] = 0
            index+=1
        elif nibbles[index] == 2:
            nibbles[index] = 3
            index+=1
        elif nibbles[index] == 3:
            nibbles[index] = 1
            index+=1

    ans = bytearray( [ nibbles[0], nibbles[1], nibbles[2], nibbles[3] ] )
...
def dec_subtract_last_two_first_two( nibbles ):
   ...
   # STUDENTS YOU MUST WRITE THIS FUNCTION
    ...
    new_nibs1 = (nibs1 - nibs3)%4
    new_nibs2 = (nibs2 - nibs4)%4

    ans = bytearray( [ new_nibs1, new_nibs2, nibs3, nibs4 ] )
